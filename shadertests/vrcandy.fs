...
float dist(vec3 p) //distance function
{
	...
	for (int i=0; i<3; i++) // 4 layers of arbitrary rotated, repeating domain...
	{
		vec3 rp = p;
		...
		c0 = mix(c0,c1,cos(iGlobalTime*0.6+float(i)*3.14159*0.5+(p.x+p.y+p.z)*0.1)*0.5+0.5);
		//mix
...
//	uv.x *= iResolution.x/iResolution.y;
	vec2 m = vec2(0.0, 0.0); //  iMouse.xy/iResolution.xy - 0.5;

	vec3 p = fragRayOri; // vec3(sin(iGlobalTime*0.1)*2.0,sin(iGlobalTime*0.21)*2.0,iGlobalTime);
	vec3 d = fragRayDir; //  vec3(uv,1.0); 
